...
class SubstationurlController extends AdminBasicController
{
	...
	public function ajaxAction()
	{
		...
		$total=$this->m_substation_url->Where($where)->Total();
        ...
        if ($total > 0) {
			...
			$sql = "SELECT * FROM `t_substation_url` Order by id desc LIMIT {$limits}";
			$items=$this->m_substation_url->Query($sql);
            if (empty($items)) {
    ...
    public function stateajaxAction()
    {
        if ($this->AdminUser==FALSE AND empty($this->AdminUser)) {
            $data = array('code' => 1000, 'msg' => '请登录');
            Helper::response($data);
        ...
        $r = $this->m_substation_url->UpdateByID(array('state'=>$state), $id);
        if ($r) {
    ...
    public function addajaxAction()
    {
		...
		$data = array();
		$url_list =$this->m_substation_url->Field('url')
            ->Where(array(
                'url' => $url
            ))
            ->Select();
		if(!empty($url_list[0]['url'])){
            $data = array('code' => 1003, 'msg' => '新增失败');
            Helper::response($data);
        }
        $r = $this->m_substation_url->Insert(array(
            'url' => $url,
        ));
        if($r){

...
class mzfqq
{
    ...
    public function pay($payconfig,$params)
    {
        ...
        //商户KEY
        $alipay_config['key']			= $payconfig['app_secret'];
        //↑↑↑↑↑↑↑↑↑↑请在这里配置您的基本信息↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
        //签名方式 不需修改
        ...
        //支付API地址
        $alipay_config['apiurl']    = 'http://www.mazhifu.net/';
        $config = array(
            "pid" => (int)$payconfig['app_id'],//平台ID号
            "type" => 'qqpay',//支付方式
            ...
            "notify_url" => $params['weburl'] . '/product/notify/?paymethod=mzfqq',
            "name" =>$params['productname'],//付款页面风格
        );

        //建立请求
        $alipaySubmit = new AlipaySubmit($alipay_config);
        $html_text = $alipaySubmit->buildRequestForm($config);
        //var_dump($html_text);
        return array('code'=>1, 'data'=>array('html'=>$html_text));
    }
    ...
    public function notify($payconfig)
    {
        ...
        //支付API地址
        $alipay_config['apiurl']    = 'http://www.mazhifu.net/';
        file_put_contents(YEWU_FILE, CUR_DATETIME.'-'.json_encode($_GET).PHP_EOL, FILE_APPEND);
        //计算得出通知验证结果
        $alipayNotify = new AlipayNotify($alipay_config);
        $verify_result = $alipayNotify->verifyNotify();

        if (!$verify_result) { //不合法的数据 KEY密钥为你的密钥
            return 'error|Notify: auth fail';
        } else { //合法的数据
            //业务处理
            $config = array('paymethod'=>$this->paymethod,'tradeid'=>$_GET['trade_no'],'paymoney'=>$_GET['money'],'orderid'=>$_GET['out_trade_no'] );
            $notify = new \Pay\notify();
